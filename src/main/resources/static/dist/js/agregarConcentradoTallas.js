function guardarTallaPrenda() {
    ...
    $("#idPedido").val(window.location.pathname.split('/')[2]);
    ...
    $.ajax({
        ...
        success: async function (data) {
            ...
            const response = await getTallasByIdEmpleado(idPedido, idEmpleado);
...
async function getTallasByIdEmpleado(idPedido, idEmpleado) {
    let result;
    try {
        result = await $.ajax({
            type: "GET",
            url: `/api/editar-concentrado-de-tallas/${idPedido}/${idEmpleado}`,
        });
        return result;
    } catch (error) {
        console.error(error);
...
function eliminarPrendaTalla(idConcentradoTalla) {
    const { idEmpleado, idPedido } = getAllFields();
    Swal.fire({
