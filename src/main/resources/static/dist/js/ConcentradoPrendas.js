			...
			success: (data) => {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if (indexPrendaEmpleado < 0) {
						$('#ListaModelos').append("<option value='" + data[i][0] + "'  data-tipo='" + data[i][4] + "'>" + data[i][1] + " - " + data[i][2] + "</option>");
					}
				}
...
function AgregarPrendaEmpleado() {
	...
	var temp = {
		...
		tipo: $('#ListaModelos').children('option:selected').data('tipo')
	}
	...
	if (!repetido) {
		...
		$.ajax({
			...
			success: (data) => {
				...
				var ModeloTemp = { id: $('#ListaModelos').val(), nombre: $("#ListaModelos option:selected").text(), tipo: $('#ListaModelos').children('option:selected').data('tipo') }
...
function QuitarPrendaEmpleado(identidad, modelo) {
	...
	console.log(ModelosAgregados[modeloEliminado]);
	$('#ListaModelos').append("<option value='" + ModelosAgregados[modeloEliminado]["id"] + "'  data-tipo='" + ModelosAgregados[modeloEliminado]['tipo'] + "'>" + ModelosAgregados[modeloEliminado]["nombre"] + "</option>");
	$('#ListaModelos').selectpicker('refresh');
