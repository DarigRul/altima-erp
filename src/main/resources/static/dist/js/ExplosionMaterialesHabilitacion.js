...
$(document).ready(function () {
	...
	//console.log(myTable2.row(5).data()[0]);
	myTable2.row(5).data()[0]="nosefas";
	// myTable2.row(5).invalidate();
	//console.log(myTable2.row());
	var tr = $('#tablaexplosion tbody tr:eq(5)');
	// var tr22=myTable2.cell(5);
	// console.log(tr22);
	// console.log(tr);
	// myTable2.row.add(1).node().id = 'myId';
	// myTable2.draw( false );
	...
	.draw();
	// console.log(myTable2.row(5).data()[0]);
	// console.log(myTable2.rows().data());
	// console.log(myTable2.data());
	// console.log($(".table tablaGeneral table-striped
	// table-bordered").DataTable().rows().data());
	...
	if(
		...
		for ( var i in arrayReg) {
			...
			tablaMultialmacenes.row.add( [
				...
				'<button id="modalTomar" onclick="contador('+i+','+(i)+','+(i)+');inputapartados('+i+');" class="btn btn-altima btn-sm btn-circle popoverxd"     data-placement="top" data-content="Tomar"><i class="fas fa-hand-pointer"></i></button>',
		...
		success: (data) => {
			...
			for (i in data) {
				...
				a = [
					...
					/*
					 * '<button id="modalTomar"
					 * onclick="inputapartados(tablaMultialmacenes.row(
					 * $(this).parents('+tr2+') ),'+separateddata+','+surtir+')"
					 * class="btn btn-altima btn-sm btn-circle popoverxd"
					 * data-placement="top" data-content="Tomar"><i class="fas
					 * fa-hand-pointer"></i></button>'+
					 */
...
function inputapartados(	
	...
	inputValidator: (value) => {
		...
		if (!value) {
		...
		else if (parseInt(value)>disponible2 || parseInt(value)>surtir2 || parseInt(value)>parseInt(disponible_existencia_almacen)) {
			// console.log(value);
			// console.log(disponible2);
			// console.log(surtir2);
			// console.log(disponible_existencia_almacen);

			return 'Ingrese un valor valido la cantidad a apartadar no puede ser mayor a la cantidad disponible: '+disponible2+' , a la cantida a surtir: '+surtir2+' o a la cantidad exitente es este almacen: '+disponible_existencia_almacen+''
		} 
		else{
			...
			var disponible3=parseInt(document.getElementById("disponible").innerHTML);
			document.getElementById("disponible").innerHTML=disponible3-arrayReg[contador1[0]].apartados;
			var surtir3=parseInt(document.getElementById("surtir").innerHTML);
