	...
	public List<Object> findAllWithTotal(Long idAgente) {
		if (idAgente==null) {
			return em.createNativeQuery("SELECT cc.id_cotizacion,  \n" + 
										...
										"			ifnull(cliente.apellido_materno,'')) AS cliente,  \n" + 
										"		(SELECT SUM(subtotal+descuento_monto+iva_monto) FROM alt_comercial_cotizacion_total \n" + 
										"				WHERE id_cotizacion=cc.id_cotizacion) AS total,  \n" + 
										"		CONCAT(empleado.nombre_persona,' ', empleado.apellido_paterno,' ', empleado.apellido_materno) AS vendedor,  \n" + 
										"		cc.estatus,\n" + 
										"		ct.anticipo_porcentaje,\n" + 
										"		ct.descuento_porcentaje,\n" + 
										"		(SELECT IF((SELECT COUNT(*)\n" + 
										"			FROM alt_comercial_cotizacion_prenda\n" + 
										"			WHERE \n" + 
										"			(precio_bordado != 0\n" + 
										"			OR porcentaje_adicional != 0\n" + 
										"			OR monto_adicional != 0)\n" + 
										"			AND id_cotizacion = cc.id_cotizacion)>0, 1 , 0)) AS variacion_precio\n" + 
										"											  \n" + 
