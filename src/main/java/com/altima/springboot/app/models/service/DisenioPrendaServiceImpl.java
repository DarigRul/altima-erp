...
public class DisenioPrendaServiceImpl implements IDisenioPrendaService {
	...
	public String[] getExistencias(Long familiaPrenda) {
		...
		//Consecutivo Numero
		List<BigInteger> existencias = em.createNativeQuery("SELECT COUNT(DP.id_prenda) FROM alt_disenio_prenda AS DP INNER JOIN alt_disenio_lookup AS DL ON DP.id_familia_prenda = DL.id_lookup WHERE DL.nombre_lookup LIKE '%" + response[1] + "%' AND DP.prenda_local != 0 AND DP.estatus = 1;").getResultList();
	...
	public List<Object[]> BuscarPrendaById(Long id) {
		...
		List<Object[]> re = em
				.createNativeQuery("SELECT adp.id_prenda, adp.id_familia_prenda, adp.id_text, adp.id_text_prospecto, adp.numero_prenda, \r\n" + 
						...
						"			WHERE 1=1 AND adp.estatus ='1' AND id_prenda="+id).getResultList();
	...
	public void integrate(
	    ...
	    metadata.getSqlFunctionMap().put("group_concat", (SQLFunction) new StandardSQLFunction( "group_concat", StandardBasicTypes.STRING)); 
	...
	public List<Object[]> ListaClientesPrenda(Long id) {
		...
		List<Object[]> re = em
				.createNativeQuery("SELECT adpc.id_cliente, GROUP_CONCAT(DISTINCT(CONCAT_WS(acc.nombre,' ', acc.apellido_paterno, ' ', acc.apellido_materno))) AS 'Clientes' \r\n" + 
						"			FROM alt_disenio_prenda_cliente adpc INNER JOIN alt_disenio_prenda adp ON adpc.id_prenda = adp.id_prenda \r\n" + 
						"			INNER JOIN alt_comercial_cliente acc ON adpc.id_cliente = acc.id_cliente WHERE 1=1 AND adpc.estatus='1' AND adp.id_prenda="+id)
				.getResultList();
	...
	public List<Object[]> ImagenPrenda(Long id) {
		...
		List<Object[]> re = em
				.createNativeQuery("SELECT adpi.id_prenda, adpi.nombre_prenda, adpi.ruta_prenda \r\n" + 
						"			FROM alt_disenio_prenda_imagen adpi INNER JOIN alt_disenio_prenda adp ON adpi.id_prenda = adp.id_prenda \r\n" + 
						"			WHERE 1=1 AND adp.id_prenda="+id)
				.getResultList();
	...
	public List<Object[]> ListaMarcadoresPrendas(Long id) {
		...
		List<Object[]> re = em
				.createNativeQuery("SELECT adp.id_prenda, GROUP_CONCAT(adl.nombre_lookup) \r\n" + 
