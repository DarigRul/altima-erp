...
public class GerencialComercialSolicitudController {
	...
	public String listPre(Model model) {
		...
		model.addAttribute("listSucur", ClienteService.findAll(null));
	...
	public String guardarPrePedido(@ModelAttribute ProduccionPedido formpedido, RedirectAttributes redirectAttrs,
		...
		System.out.println(formpedido);
		servicePedido.save(formpedido);

		DisenioNotificacion dn = new DisenioNotificacion();
		if (formpedido.getTipoPedido().equals("2")) {
			dn.setAsunto("Pedido Coleccion");
		} else {

			dn.setAsunto("Pedido Gerencial");

		}
		dn.setFecha(formpedido.getFechaEntrega());
		dn.setCliente(formpedido.getIdCliente().toString());
		dn.setEstatus(0);
		dn.setIdSolicitud(formpedido.getIdPedido());
		NotificacionService.save(dn);
		dn.setFolio("FOL00" + dn.getIdNotificacion());
		NotificacionService.save(dn);
	...
	public Integer guardar(@RequestParam(name = "datosMateriales") String datosMateriales,
			...
			Long idPrenda, Long idModelo, int cantidad, Long idTela, Long idGeneroinsert, Long idPedido, String idLargo,
			HttpServletRequest request, String idTalla) {
	...
	public String eliminarColeccion(Model model, @PathVariable(value = "id") Long id,
	...
	public String eliminarSolicitudColeccion(@PathVariable(value = "id") Long id,
			@PathVariable(value = "idp") Long idp) {
		// serviceDetallePedido.deleteByIdDetalle(id, idp);
