...
public class HrPermisoController {
	...
	public String permisos(Model model) {
	   ...
	   //con el findAll puedo acceder a sus metodos sin necesidad de hacer consultas accedo a la inf
		...
		//tengo la informacion y es la que mando en el html para que se muestre en la vista
		public String guardarPermisos( Model model,@RequestParam("idEmpleado") Long empleado,@RequestParam("tipoPermiso") String tipoPermiso,
				...
				@RequestParam("horaInicial") String horaInicial,@RequestParam("horaFinal") String horaFinal,
				@RequestParam("observaciones") String observaciones,RedirectAttributes redirectAttrs) {
			List<HrPermiso> rhPermiso = servicePermiso.findListaPermiso();
			model.addAttribute("listarPermisos", rhPermiso);
			...
			//redirectAttrs.addFlashAttribute("title", "Permiso Insertado Correctamente").addFlashAttribute("icon","primary");
			return "/rh-permisos";
		...
		//tengo la informacion y es la que mando en el html para que se muestre en la vista
		public String editarPermisos( Model model,@RequestParam("idEmpleado") Long empleado,@RequestParam("tipoPermiso") String tipoPermiso,
				@RequestParam("evento") String evento,
				@RequestParam("condiciones") String condiciones,@RequestParam("fechaInicial") String fechaInicial,
				@RequestParam("fechaFinal") String fechaFinal,
				@RequestParam("horaInicial") String horaInicial,@RequestParam("horaFinal") String horaFinal, @RequestParam("id_permiso") String id,
		...
		public String eliminar(@PathVariable(value = "id") Long id, Model model, Map<String, Object> m, RedirectAttributes redirectAttrs) {
		...
		public String rechazarPermiso(@PathVariable(value = "id") Long id, Model model, Map<String, Object> m) {
			...
			//encuentro los datos en este caso el id 
		 	HrPermiso addPermiso = servicePermiso.findOne(id);
		 	//el estatus por el cual debo actualizar mi campo en este caso activar o rechazar 
		 	addPermiso.setEstatusPermiso("0");
		 	//cuando le doy al boton me debe regresar a la vista a donde listo usando los mismo dato de mi metodo listar 
		 	List<HrPermiso> rhPermiso = servicePermiso.findListaPermiso();
			model.addAttribute("listarPermisos", rhPermiso);
